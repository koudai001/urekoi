...
- 保守性を優先し、CSS・Tailwindクラスは見た目と操作性に必要な最小限に抑える。装飾目的のクラスや細かな状態別スタイルを安易に増やさず、シンプルな構成を優先する
- レイアウトに不要なwrapperを増やさず、効果のない`flex`・`flex-1`・固定高さ・重複paddingなどのCSSクラスは追加しない。まず最小限のHTMLとクラスで実装する
